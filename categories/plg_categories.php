 ...
 * @Modification Griboval Cyrille 
 * @gestion catégorie mére/fille 
 **/
		...
		<table id="categories-table" class="full-width plg_table" data-rows-num='name$="_ordre"'>
			...
			if($plxAdmin->aCats) {
				foreach($plxAdmin->aCats as $k=>$v) { # Pour chaque catégorie
					...
					echo '</td><td>';
					if($v['daughterOf'] !=='000') {
						plxUtils::printSelect($k.'_mother',      array('0'=>L_NO), $v['mother'],'','not-active');
					} else {
					...
					echo '</td><td>';
					if ($v['mother'] === "1" ) {
						plxUtils::printSelect($k.'_daughterOf', array('000'=>'orpheline'), $v['daughterOf'],'','not-active');
					} else { 
